...
class PagedTable extends Component {
  ...
  renderPageNumbers() {
    ...
    for (let i = 0; i < this.numOfPages; i++) {
      output.push(
        <li key={i} className={`page-item ${this.currentPageNum === i + 1 ? 'active' : ''}`} onClick={this.goToPage.bind(this, i + 1)}>
  ...
  render() {
    ...
    return (
      <div style={{ position: 'relative' }}>
        ...
        <table className={`table paged-table ${this.props.tableClassName}`} style={{ opacity: this.props.isLoading ? 0.6 : 1 }}>
          <thead>{heading}</thead>
            ...
            <tr>
              <td colSpan={heading.props.children.length}>
                <ul className="pagination justify-content-end">
                  <li onClick={this.goToPage.bind(this, 1)} className={`page-item ${this.isCurrentPageAtMin() ? 'disabled' : ''}`}>
                    <button className="page-link" type="button" tabIndex="-1">
                      {'<<'}
                    </button>
                  </li>
                  <li onClick={this.goPreviousPage.bind(this)} className={`page-item ${this.isCurrentPageAtMin() ? 'disabled' : ''}`}>
                    <button className="page-link" type="button" tabIndex="-1">
                      {'<'}
                    </button>
                  </li>
                  {pageNumbers}
                  <li onClick={this.goNextPage.bind(this)} className={`page-item ${this.isCurrentPageAtMax() ? 'disabled' : ''}`}>
                    <button className="page-link" type="button">
                      {'>'}
                    </button>
                  </li>
                  <li onClick={this.goToPage.bind(this, this.numOfPages)} className={`page-item ${this.isCurrentPageAtMax() ? 'disabled' : ''}`}>
                    <button className="page-link" type="button">
                      {'>>'}
                    </button>
                  </li>
